...
import { getAllFactNonValiderAsync } from "../../../redux/facture/FactureSlice.js";
...
const ListNonValider = () => {
    ...
    useEffect(() => {
        dispatch(getAllFactNonValiderAsync());
    }, [dispatch]);
            ...
            renderCell: (params) => (
                <Box display="flex" alignItems="center" gap={1} marginTop="8px">
                    <Button
                        variant="outlined"
                        color="secondary"
                        onClick={() => navigate(`/valider-facture/${params.row.id}`, { state: { bordereau: params.row } })}
                    >
                        Valider
                    </Button>
                    {/*<Button*/}
                    {/*    variant="outlined"*/}
                    {/*    color="error"*/}
                    {/*    onClick={() => navigate(`/rejeter-bordereau/${params.row.id}`)}*/}
                    {/*>*/}
                    {/*    Rejeter*/}
                    {/*</Button>*/}
                </Box>
